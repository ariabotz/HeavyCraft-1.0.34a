...
async function stablediffusion(positivePrompt, negativePrompt) {
return new Promise(async (resolve, reject) => {
try {
const response = await axios.post('https://camenduru-webui-docker.hf.space/run/predict/', {
"fn_index": 48,
"data": [
positivePrompt, negativePrompt, "None", "None", 20, "DPM++ 2M Karras", false, false, 1, 1, 7, -1, -1, 0, 0, 0, false, 960, 720, false, 0.7, 0, 0, "None", null, "", ""
]
});
resolve('https://camenduru-webui-docker.hf.space/file=' + response.data.data[0][0].name);
} catch (error) {
reject(error);
}
});
...
module.exports = { pinterest, wallpaper, wikimedia, quotesAnime, happymod, umma, ringtone, stablediffusion, jadwalsholat, styletext }
